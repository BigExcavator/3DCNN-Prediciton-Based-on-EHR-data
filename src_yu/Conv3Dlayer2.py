...
class Conv3Dlayer(object):
    ...
    def __init__(self, rng, input, filter_shape):
        ...
        fan_out = numpy.prod(filter_shape[0]) * numpy.prod(filter_shape[3:])
        #initialize weights (if 'tanh' is the activation function)
        W_bound = numpy.sqrt(6.  / (fan_in + fan_out))
        
        
        P = rng.uniform(low = -W_bound, high = W_bound , size = (filter_shape[0], filter_shape[2], filter_shape[3], filter_shape[4]) )
        
          
        Q = numpy.zeros(filter_shape)
